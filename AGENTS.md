...
- **`src/armv7m_decoder/_itstate.py`** — ITSTATE tracking (`next_itstate`, `current_cond`, `in_it_block`) for carrying an IT block's condition across a stream. All internal, used by the CLI and `_disasm`; only the constant `COND_AL` is public, and only because a decoded instruction's `cond` member can hold it.
...
src/armv7m_decoder/         # Source package
  ...
  _word.py                  # The instruction word: its size, and both ways in
